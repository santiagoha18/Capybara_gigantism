...
skeleton <- t2[t2$gene_name=="SMAD6" |  t2$gene_name=="ACVR1B" | t2$gene_name=="BMPR1A" | t2$gene_name=="DEAF1" | t2$gene_name=="TAPT1" , ]
cell_prol_growth <- t2[t2$gene_name=="PLAGL2", ]
limb <- t2[t2$gene_name=="OSR1" | t2$gene_name=="MEOX2", ]
label_insulin <- t2[t2$gene_name=="Ins",]


ggplot(t2, aes(LR,D_stat_prot_Stand)) + geom_point(colour="grey87") + geom_point(data=labels, aes(x=LR, y=D_stat_prot_Stand), colour="#666666") + geom_point(data=skeleton, aes(x=LR, y=D_stat_prot_Stand), colour="#990066",size=3) + geom_point(data=cell_prol_growth, aes(x=LR, y=D_stat_prot_Stand), colour="#FFCC00",size=3) + geom_point(data=limb, aes(x=LR, y=D_stat_prot_Stand), colour="#3399FF",size=3) + geom_point(data=label_insulin, aes(x=LR, y=D_stat_prot_Stand), colour="darkorange1",size=3) + geom_text_repel(data=rbind(skeleton,cell_prol_growth,limb,label_insulin),aes(label=gene_name),fontface = 'bold', color = 'black', box.padding = unit(0.35, "lines"), point.padding = unit(0.5, "lines")) + labs(x="LR branch-site test", y = "Standardized PDI") + theme(panel.background = element_rect(fill = "white", colour = "black"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text.x = element_text(face="bold", size=13),axis.text.y = element_text(face="bold", size=13), axis.title.y = element_text(face="bold", size=15),axis.title.x = element_text(face="bold", size=15)) + theme(legend.position = c(0.75, 0.6),legend.background = element_rect(color = "black", fill = "white", size = 1, linetype = "solid")) + geom_vline(xintercept = quantile(t2$LR,0.95), linetype="dotted") + geom_hline(yintercept = quantile(t2$D_stat_prot_Stand,0.95), linetype="dotted") + ylim(0,40)
