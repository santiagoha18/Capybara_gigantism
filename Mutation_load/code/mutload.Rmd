...
labels_rate <- c(
  "median_omega" = expression(omega), 
  "median_dn" = "dN", 
  "median_ds" = "dS"
)

lnBM <- rep(tab$ln.masa,3)
tab2 <- dplyr::select(tab,median_omega,median_dn,median_ds) %>% melt()
tab2$BM <- lnBM
x <- ggplot(tab2,aes(BM, value, shape=variable,colour=variable, fill=variable)) +
  geom_smooth(method="lm") +
  geom_point(size=3) + ggtitle("Body mass and molecular rates") + xlab("Ln(BM)") + ylab("Molecular rate") + scale_shape(labels = labels_rate) + 
  scale_colour_discrete(labels = labels_rate) + 
  guides(fill = "none")

lnGT <- rep(tab$ln.gen_time,3)
tab3 <- dplyr::select(tab,median_omega,median_dn,median_ds) %>% melt()
tab3$GT <- lnGT
y <- ggplot(tab3,aes(GT, value, shape=variable,colour=variable, fill=variable)) +
  geom_smooth(method="lm") +
  geom_point(size=3) + ggtitle("Generation time and molecular rates") + xlab("Ln(GT)") + ylab("Molecular rate") + scale_shape(labels = labels_rate) + 
  scale_colour_discrete(labels = labels_rate) + 
  guides(fill = "none")
...
lnBM <- rep(tab2$ln.masa,2)
tab3 <- dplyr::select(tab,median_dn,median_ds) %>% melt()
tab3$BM <- lnBM
e <- ggplot(tab3,aes(BM, value, shape=variable, fill=variable, colour=variable)) +
  geom_smooth(method="lm",colour="black", fill = "gray") +
  geom_point(size=3) + xlab("Ln(BM)") + ylab("Molecular distance") + scale_shape_manual(labels = labels_rate, values = c(19,17)) + 
  scale_color_manual(labels = labels_rate, values=c("#0072B2","#D55E00")) +
...
ggarrange(b,c,e, ncol = 3, nrow = 1,labels = c("A","B","C"))
